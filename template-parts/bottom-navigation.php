<?php
/**
 * Bottom Navigation Template Part
 * Barra di navigazione fissa in basso - versione Tailwind CSS
 */

$current_page = '';
if (is_post_type_archive('contest') || is_home() || is_front_page()) {
    $current_page = 'concorsi';
} elseif (is_page('classifica')) {
    $current_page = 'classifica';
} elseif (is_page('regolamento')) {
    $current_page = 'regolamento';
} elseif (is_page('profilo') || is_author()) {
    $current_page = 'profilo';
}
?>

<!-- Bottom Navigation - Solo versione unificata -->
<nav id="bottom-nav" class="fixed bottom-0 w-full bg-white border-t border-gray-200 z-50">
    <div class="flex justify-around py-3">
        
        <!-- Concorsi/Home -->
        <a href="<?php echo get_post_type_archive_link('contest'); ?>" 
           class="flex flex-col items-center <?php echo ($current_page === 'concorsi') ? 'text-blue-500' : 'text-gray-600'; ?>">
            <i class="fa-solid fa-trophy text-xl mb-1"></i>
            <span class="text-xs">Concorsi</span>
        </a>
        
        <!-- Classifica -->
        <a href="<?php echo get_permalink(get_page_by_path('classifica')); ?>" 
           class="flex flex-col items-center <?php echo ($current_page === 'classifica') ? 'text-blue-500' : 'text-gray-600'; ?>">
            <i class="fa-solid fa-chart-bar text-xl mb-1"></i>
            <span class="text-xs">Classifica</span>
        </a>
        
        <!-- Regolamento -->
        <a href="<?php echo get_permalink(get_page_by_path('regolamento')); ?>" 
           class="flex flex-col items-center <?php echo ($current_page === 'regolamento') ? 'text-blue-500' : 'text-gray-600'; ?>">
            <i class="fa-solid fa-file-lines text-xl mb-1"></i>
            <span class="text-xs">Regolamento</span>
        </a>
        
        <!-- Profilo -->
        <?php if (is_user_logged_in()): ?>
            <a href="<?php echo get_permalink(get_page_by_path('profilo')); ?>" 
               class="flex flex-col items-center relative <?php echo ($current_page === 'profilo') ? 'text-blue-500' : 'text-gray-600'; ?>">
                <div class="relative">
                    <?php echo get_avatar(get_current_user_id(), 24, '', '', array('class' => 'w-6 h-6 rounded-full mb-1')); ?>
                    
                    <!-- Badge punti -->
                    <?php 
                    $user_points = instacontest_get_user_points(get_current_user_id());
                    if ($user_points > 0): 
                    ?>
                        <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center text-xs font-bold"><?php echo $user_points > 99 ? '99+' : $user_points; ?></span>
                    <?php endif; ?>
                </div>
                <span class="text-xs">Profilo</span>
            </a>
        <?php else: ?>
            <a href="<?php echo wp_login_url(get_permalink()); ?>" 
               class="flex flex-col items-center text-gray-600">
                <i class="fa-solid fa-user text-xl mb-1"></i>
                <span class="text-xs">Accedi</span>
            </a>
        <?php endif; ?>
        
    </div>
</nav>

<!-- Spacer per evitare che il contenuto sia coperto dalla nav -->
<div class="pb-20"></div>
